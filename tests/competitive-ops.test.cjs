...
test('guest covers pax on cart bill KOT Z', () => {
  ...
  const ops = fs.readFileSync(path.join(root, 'assets/competitive-ops.js'), 'utf8');
  const receipt = fs.readFileSync(path.join(root, 'assets/receipt.js'), 'utf8');
  assert.match(html, /cart-covers|id="cart-covers"/);
  assert.match(posUi, /getCovers|setCovers/);
  assert.match(pos, /covers:|pax:/);
  assert.match(ops, /coversTotal|avgPerCover/);
  assert.match(receipt, /covers/);
});
