...
(function (global) {
  ...
  // GSTR CSV download — richer columns for accountant handoff + live progress
  ...
  if (gstrBtn) {gstrBtn.onclick = () => {
    ...
    const prog =
      global.RSProgress &&
      RSProgress.open({
        title: 'Exporting GSTR CSV…',
        sub: 'Building accountant file from paid bills',
    ...
    try {
      ...
      paidBills.forEach((b, idx) => {
        ...
        if (prog && (idx % 15 === 0 || idx === paidBills.length - 1)) {
          prog.update({
            done: idx + 1,
            ...
            sub:
              ...
              paidBills.length +
              ' · ' +
              Math.max(0, paidBills.length - idx - 1) +
              ' remaining',
          });
        }
      });
      ...
      if (global.RS && typeof RS.downloadFile === 'function') {
        RS.downloadFile(csv, 'text/csv;charset=utf-8;', 'GSTR_report_' + new Date().toISOString().slice(0, 10) + '.csv');
      } else {
      ...
      if (prog) {
        prog.succeed('GSTR CSV ready · ' + paidBills.length + ' bills');
        prog.close(900);
      }
      ...
      if (prog) {
        prog.fail(e.message || 'GSTR export failed');
        prog.close(2200);
      }
      toast('GSTR export failed', 'fa-circle-exclamation');
    }
  };}
  }
